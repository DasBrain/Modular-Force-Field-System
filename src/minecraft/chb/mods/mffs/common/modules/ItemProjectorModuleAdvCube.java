        ...
        for (int z1 = 0 - zPout; z1 < zMout + 1; z1++) {
          ...
          if ((y1 == 0) || (y1 == Strength) || (x1 == 0 - xMout) || (x1 == xPout) || (z1 == 0 - zPout) || (z1 == zMout))
          {
            if (((TileEntityProjector)projector).hasOption(ModularForceFieldSystem.MFFSProjectorOptionDome, true))
            {
              ...
              {
              case 0:
                if (((TileEntityProjector)projector).yCoord + tpy <= ((TileEntityProjector)projector).yCoord) break;
                break;
              case 1:
                if (((TileEntityProjector)projector).yCoord + tpy >= ((TileEntityProjector)projector).yCoord) break;
                break;
              case 2:
                if (((TileEntityProjector)projector).zCoord + tpz <= ((TileEntityProjector)projector).zCoord) break;
                break;
              case 3:
                if (((TileEntityProjector)projector).zCoord + tpz >= ((TileEntityProjector)projector).zCoord) break;
                break;
